...
\begin{definition}[Square patch]\label{def:square:patch}
  When in the case of $4$-valent polyhedrons, a square patch denotes a $4$-patch witch boundary structure consisting four times of the face count $1$, while every other vertex has has face count $2$.
\end{definition}
...
Each $k$-gon of the sequence $(k > 5)$ can be grouped together with $k-4$ triangles to create a ``flat'' block (they satisfy \autoref{eq:zero:curv:4}). This can be brought to an square patch by adding $2 \times 2$ building blocks, as well as some additional triangles and pentagons as seen in the following picture.
...
\begin{theorem}\label{thm:case3:5} Let $p = (p_3, p_4, p_5, \dots, p_n)$ be a given sequence satisfying \autoref{eq:valence:4}. Then $p$ is $[3, 5]$-$4$-realizable.
  \begin{proof}
    ...
    This polyhedron with $p$-vector $p''$ satisfies 
    \begin{align*}
